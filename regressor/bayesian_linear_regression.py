...
class discriminative_bayesian_linear_regression:
    def __init__(self, sigma2_simulateur,mu_theta=torch.tensor(0.), sigma2_theta=torch.tensor(1.)):
...
class generative_bayesian_linear_regression:
    ...
    def compute_x0_given_y0_theta_moments(self, y0, theta):
        sigma2_x0_given_y0_theta = 1 /(1/self.sigma2_X + y0.shape[0]*torch.square(theta)/self.sigma2_simulateur)
        mu_x0_given_y0_theta = sigma2_x0_given_y0_theta*(self.mu_X/self.sigma2_X + theta * torch.sum(y0) / self.sigma2_simulateur)
        return mu_x0_given_y0_theta, sigma2_x0_given_y0_theta
    ...
    def sample_x0_given_y0_D_Y_gibbs(self, y0,DX, DY,Y = torch.tensor([]),prior_means = torch.tensor([]),prior_sigma2s = torch.tensor([]),number_steps = 100,verbose = False):
        ...
        for _ in pbar:
            ...
            for y, prior_mean, prior_sigma2 in zip(Y, prior_means, prior_sigma2s):
                sigma2_xj_given_yj_theta = 1 / (
                            1 / prior_sigma2 + y.shape[0] * torch.square(current_theta) / self.sigma2_simulateur)
                mu_xj_given_yj_theta = sigma2_xj_given_yj_theta * (
                            prior_mean / prior_sigma2 + current_theta * torch.sum(y) / self.sigma2_simulateur)
                current_label = torch.distributions.Normal(mu_xj_given_yj_theta, torch.sqrt(sigma2_xj_given_yj_theta)).sample()
    ...
    def sample_x0_given_y0_D_Y_gibbs_(self, y0,DX, DY,Y = torch.tensor([]),prior_means = torch.tensor([]),prior_sigma2s = torch.tensor([]),number_steps = 100,verbose = False):
        ...
        for _ in pbar:
            ...
            for y, prior_mean, prior_sigma2 in zip(Y, prior_means, prior_sigma2s):
                ...
                mu_xj_given_yj_theta = sigma2_xj_given_yj_theta * (
                        prior_mean / prior_sigma2 + current_theta * torch.sum(y) / self.sigma2_simulateur)
                current_label = torch.distributions.Normal(mu_xj_given_yj_theta,
                                                           torch.sqrt(sigma2_xj_given_yj_theta)).sample()
                current_labels.append(current_label.repeat(y.shape[0]))
            DXplus = torch.cat([DX, current_x0.repeat(y0.shape[0]), torch.cat(current_labels)], dim=0)
            list_x0_gibbs.append(current_x0)
            list_theta_gibbs.append(current_theta)
...
class generative_bayesian_affine_regression:
    ...
    def compute_x0_given_y0_theta_moments(self, y0, theta):
        sigma_x0_given_y0_theta = 1 / (
                    1 / self.sigma2_X + (y0.shape[0] * torch.square(theta[0])) / self.sigma2_simulateur)
    ...
    def sample_x0_given_y0_D_Y_gibbs_(self, y0,DX, DY,Y = torch.tensor([]),prior_means = torch.tensor([]),prior_sigma2s = torch.tensor([]),number_steps = 100,verbose = False):
        ...
        for _ in pbar:
            ...
            for y, prior_mean, prior_sigma2 in zip(Y, prior_means, prior_sigma2s):
                sigma2_xj_given_yj_theta = 1 / (
                        1 / prior_sigma2 + (y.shape[0] * torch.square(current_theta[0])) / self.sigma2_simulateur)
                mu_xj_given_yj_theta = sigma2_xj_given_yj_theta * (
                        prior_mean / prior_sigma2 + current_theta[0] * torch.sum(y - current_theta[1]) / self.sigma2_simulateur)
                current_label = torch.distributions.Normal(mu_xj_given_yj_theta,
                                                           torch.sqrt(sigma2_xj_given_yj_theta)).sample()
                current_labels.append(current_label.repeat(y.shape[0]))
            DXplus = torch.cat([DX, current_x0.repeat(y0.shape[0]), torch.cat(current_labels)], dim=0)
