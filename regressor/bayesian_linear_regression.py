...
class discriminative_bayesian_affine_regression_known_variance:
    ...
    def compute_beta_given_D_moments(self, DX, DY):
        ...
        Sigma_beta_given_D = torch.inverse(temp @ temp / self.sigma2_simulateur + torch.inverse(self.Sigma_beta))
...
class generative_bayesian_affine_regression_known_variance:
    ...
    def sample_x0_given_y0_D_Y_gibbs(self, y0,DX, DY,Y = torch.tensor([]),prior_means = torch.tensor([]),prior_sigma2s = torch.tensor([]),number_steps=100,verbose = False):
        ...
        mean_x0_given_y0_beta, sigma2_x0_given_y0_beta = self.compute_x0_given_y0_beta_moments(y0, current_beta)
        current_x0 = torch.distributions.Normal(mean_x0_given_y0_beta, torch.sqrt(sigma2_x0_given_y0_beta)).sample()
        list_x0_gibbs = []
        list_beta_gibbs = []
        if verbose:
            pbar = tqdm(range(number_steps))
        ...
        for _ in pbar:
            mean_x0_given_y0_beta, sigma2_x0_given_y0_beta = self.compute_x0_given_y0_beta_moments(y0, current_beta)
            current_x0 = torch.distributions.Normal(mean_x0_given_y0_beta,
                                                    torch.sqrt(sigma2_x0_given_y0_beta)).sample()
            current_labels = []
            for y, prior_mean, prior_sigma2 in zip(Y, prior_means, prior_sigma2s):
                sigma2_xj_given_yj_beta = 1 / (
        ...
        for _ in pbar:
            ...
            for y, prior_mean, prior_sigma2 in zip(Y, prior_means, prior_sigma2s):
                ...
                mu_xj_given_yj_beta_sigma2 = sigma2_xj_given_yj_beta_sigma2 * (
                        prior_mean / prior_sigma2 + current_beta[0] * torch.sum(y - current_beta[1]) / current_sigma2)
