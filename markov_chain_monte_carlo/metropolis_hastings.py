...
class IndependentMetropolisHastingsMultipleObs(torch.nn.Module):
    ...
    def independent_accept_metropolis_step(self,x, x_prime):
    ...
    def sample(self, number_steps, verbose = False):
        ...
        for _ in pbar:
            ...
            x,mask = self.independent_accept_metropolis_step(x,proposed_x)
            if verbose:
                pbar.set_postfix_str('acceptance = ' + str(torch.mean(mask * 1.)))
        return x
...
class RandomWalkMetropolisHastings(torch.nn.Module):
    ...
    def accept_metropolis_step(self,x, x_prime):
        log_target_density_ratio = self.target_log_prob(x_prime) - self.target_log_prob(x)
    ...
    def sample(self, number_steps, verbose = False):
        ...
        if verbose:
            ...
            pbar = range(number_steps)
        for _ in pbar:
            proposed_x = x + torch.randn_like(x)*torch.sqrt(torch.tensor(self.sigma2))
            x,mask = self.accept_metropolis_step(x,proposed_x)
            if verbose:
                pbar.set_postfix_str('acceptance = ' + str(torch.mean(mask * 1.)))
        return x

class RandomWalkMetropolisHastingsMultipleObs(torch.nn.Module):
    def __init__(self, log_posterior,observations,d,sigma2 = 1.,initial_distribution=None):
        super().__init__()
        self.target_log_prob = log_posterior
        self.observations = observations
        self.number_obs = observations.shape[0]
    ...
    def sample(self, number_steps, verbose = False):
        x = self.initial_distribution.sample([self.number_obs])
        if verbose:
            pbar = tqdm(range(number_steps))
        else:
            pbar = range(number_steps)
