...
from density_estimation import SoftmaxWeight, LocationScaleFlow
...
class ConditionalDIF(torch.nn.Module):
    def __init__(self, D_x, D_theta, K, hidden_dimensions):
        ...
        self.reference = torch.distributions.MultivariateNormal(self.reference_mean, self.reference_cov.reshape(self.p, self.p))
    ...
    def reference_log_prob(self,z):
        return torch.distributions.MultivariateNormal(self.reference_mean.to(z.device), self.reference_cov.reshape(self.p, self.p).to(z.device)).log_prob(z)
...
class FakeConditionalDIF(torch.nn.Module):
    def __init__(self, D_x, D_theta, K, hidden_dimensions):
        super().__init__()
        self.D_x = D_x
        self.D_theta = D_theta
    ...
    def compute_log_v(self,x, theta):
        ...
        theta_unsqueezed = theta.unsqueeze(-2).repeat(1, self.K, 1)
        z = self.T.forward(x)
        log_v = self.reference.log_prob(z) + torch.diagonal(self.W.log_prob(torch.cat([z, theta_unsqueezed], dim = -1)), 0, -2, -1) + self.T.log_det_J(x)
        return log_v - torch.logsumexp(log_v, dim = -1, keepdim= True)
    ...
    def log_prob(self, x, theta):
        ...
        theta_unsqueezed = theta.unsqueeze(-2).expand(desired_size)
        z = self.T.forward(x)
        return torch.logsumexp(self.reference_log_prob(z) + torch.diagonal(self.W.log_prob(torch.cat([z, theta_unsqueezed], dim = -1)), 0, -2, -1)+ self.T.log_det_J(x),dim=-1)
